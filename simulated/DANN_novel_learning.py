...
print(data.describe())
ourdir = f'test_roc_adv_basic_transform_sin3/{varReduction}'
...
counter = 0
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=int(patience/1), factor=0.5)
...
plt.legend()
plt.tight_layout()
fig.savefig(f'DANN_losscurve_{varReduction}.pdf')
fig.savefig(f'DANN_losscurve_{varReduction}.png')
plt.show()
...
with torch.no_grad():
    ...
    plt.tight_layout()
    plt.savefig(f'DANN_roc_curve_{varReduction}.png')
    plt.savefig(f'DANN_roc_curve_{varReduction}.pdf')
    plt.show()
