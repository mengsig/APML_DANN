...
print(data.describe())
ourdir = f'true_roc_adv_one_transform_sin3/{varReduction}'
...
for epoch in range(num_epochs):
    ...
    for i in range(0, X_train_tensor.size(0), batch_size):
        ...
        feature_extractor.train()
        combined_data = torch.cat((sim_data, noise1_data, noise2_data, real_data), 0)
...
#        combined_features = torch.cat((sim_features, noise1_features, noise2_features, real_features), 0)
        domain_labels = torch.cat((
            ...
            torch.ones(noise1_data.size(0)).long(),
            1 * torch.ones(noise2_data.size(0)).long(),
            1 * torch.ones(real_data.size(0)).long()
        )).to(device)
        ...
        epoch_classification_loss += supervised_loss.item()/3
...
plt.legend()
plt.tight_layout()
fig.savefig(ourdir + f'/DANN_one_losscurve_{varReduction}.pdf')
fig.savefig(ourdir + f'/DANN_one_losscurve_{varReduction}.png')
plt.show()
...
with torch.no_grad():
    ...
    plt.tight_layout()
    plt.savefig(ourdir + f'/DANN_one_roc_curve_{varReduction}.png')
    plt.savefig(ourdir + f'/DANN_one_roc_curve_{varReduction}.png')
    plt.show()
